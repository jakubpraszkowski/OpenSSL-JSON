...
int main(void) {
    ...
    curl = curl_easy_init();
    if (curl) {
        curl_easy_setopt(curl, CURLOPT_URL, "https://api.open-meteo.com/v1/forecast?latitude=50.33&longitude=19.13&current_weather=true");
        ...
        else
        {
            if (reader.parse(str, js))
            {
                ...
                if (js.get("latitude", NAN).isDouble()) {
                    std::cout << "Latitude:\t" << js.get("latitude", NAN).asDouble() << std::endl;
                }
                if (js.get("longitude", NAN).isDouble()) {
                    std::cout << "Longitude:\t" << js.get("longitude", NAN).asDouble() << std::endl;
                }
                std::cout << "Time zone:\t" << js.get("timezone", "null") << std::endl;


                if (js.get("current_weather", "null").isObject()) {
                    if (js.get("current_weather", "null").get("temperature", NAN).isDouble()) {
                        std::cout << "Temperature:\t" << js.get("current_weather", "null").get("temperature", NAN).asDouble() << std::endl;
                    }
                    if (js.get("current_weather", "null").get("windspeed", NAN).isDouble()) {
                        std::cout << "Wind speed:\t" << js.get("current_weather", "null").get("windspeed", NAN).asDouble() << std::endl;
                    }
                    if (js.get("current_weather", "null").get("winddirection", NAN).isDouble()) {
                        std::cout << "Wind direction:\t" << js.get("current_weather", "null").get("winddirection", NAN).asDouble() << std::endl;
                    }
                    if (js.get("current_weather", "null").get("weathercode", NAN).isDouble()) {
                        std::cout << "Weather code:\t" << js.get("current_weather", "null").get("weathercode", NAN).asDouble() << std::endl;
                    }
                    std::cout << "Time:\t\t" << js.get("current_weather", "null").get("time", "null") << std::endl;
                }
